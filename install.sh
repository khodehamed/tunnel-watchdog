...
# Open interactive menu ONLY (do not end on `tunnel-menu status`).
# curl|sudo bash has a pipe as stdin — bind all FDs to /dev/tty.
# Skip CI/automation: TUNNEL_WATCHDOG_NO_MENU=1
open_menu() {
  ...
  if [[ -r /dev/tty && -w /dev/tty ]]; then
    tunnel-menu </dev/tty >/dev/tty 2>/dev/tty || true
    return 0
  fi
  if [[ -n "${SSH_TTY:-}" && -r "${SSH_TTY}" && -w "${SSH_TTY}" ]]; then
    tunnel-menu <"${SSH_TTY}" >"${SSH_TTY}" 2>"${SSH_TTY}" || true
    return 0
  fi
  if [[ -t 0 && -t 1 ]]; then
    tunnel-menu || true
  ...
  echo "Note: no TTY — menu not opened. Run: tunnel-menu"
